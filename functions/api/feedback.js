...
async function checkRateLimit(request, env) {
  ...
  await env.FEEDBACK_DB.prepare("DELETE FROM feedback_rate_limits WHERE window_started < ?")
...
export async function onRequestGet({env}) {
  return json({ready: Boolean(env.TURNSTILE_SITE_KEY), siteKey: env.TURNSTILE_SITE_KEY || ""});
}

export async function onRequestPost({request, env}) {
  if (!env.FEEDBACK_DB || !env.TURNSTILE_SECRET_KEY || !env.RATE_LIMIT_SALT) {
